        ...
        if ($stmt->execute()) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                if (file_exists(realpath("$folderName/$IDfilename"))) {
                    ...
                    $fontPath = '../fonts/Poppins/Poppins-Italic.otf';
                    $fontSize = 30;

                    // Calculate the text box dimensions
                    $textBox = imagettfbbox($fontSize, 0, $fontPath, $participantID);
                    $textBoxWidth = abs($textBox[2] - $textBox[0]);

                    // Calculate the text position
                    $textX = imagesx($image) - 333;
                    $textY = imagesy($image) - 355;

                    // Add the text to the image
                    imagettftext($image, $fontSize, 0, (int) $textX, (int) $textY, $color, $fontPath, $participantID);

                    // Add an underline to the text
                    $underlineY = $textY + 5; // adjust the y-coordinate of the underline
                    $underlineThickness = 2; // adjust the thickness of the underline

                    // Draw the underline
                    imagesetthickness($image, $underlineThickness);
                    imageline($image, $textX, $underlineY, $textX + $textBoxWidth, $underlineY, $color);
